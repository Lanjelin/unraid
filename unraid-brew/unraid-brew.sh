...
_ub_install() {
  ...
  if ! command -v gcc >/dev/null 2>&1; then
    echo "Installing gcc"
    if [ ! -f "$UNRAID_BREW_PATH"/gcc-13.2.0_multilib-x86_64-1alien.txz ]; then
      curl --output-dir "$UNRAID_BREW_PATH" -sO http://slackware.nl/people/alien/multilib/current/gcc-13.2.0_multilib-x86_64-1alien.txz
    fi
    installpkg "$UNRAID_BREW_PATH"/gcc-13.2.0_multilib-x86_64-1alien.txz >/dev/null
  fi
