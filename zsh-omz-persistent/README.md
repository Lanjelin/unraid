...
```bash
if [[ \$- == *i* ]]; then if [ -d /mnt/user/appdata/zsh/.oh-my-zsh ] && [ -x /bin/zsh ]; then /bin/zsh; exit; fi fi
```
...
echo "if [[ \$- == *i* ]]; then if [ -d /mnt/user/appdata/zsh/.oh-my-zsh ] && [ -x /bin/zsh ]; then /bin/zsh; exit; fi fi" >>/root/.bash_profile
